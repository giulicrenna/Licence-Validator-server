...
void changeCredentials(fs::FS &fs, const char * path, String mailReceiver,
  ...
  String mqttH, String mqttP, String mqttS);
    ...
    */
    ...
    public:
        ...
        void setupServer(String staticIpAP, String gatewayAP_, String subnetMaskAP){
          ...
          myManager.setTitle("Licence Manager Server");
          ...
          WiFiManagerParameter mqttH("mqttHost", "Host Mqtt", mqttHost, 40, "required");
          myManager.addParameter(&mqttH);
          WiFiManagerParameter mqttP("mqttPub", "Publish topic", mqttPub, 40, "required");
          myManager.addParameter(&mqttP);
          WiFiManagerParameter mqttS("mqttSub", "Subscribe topic", mqttSub, 40, "required");
          myManager.addParameter(&mqttS);
          ...
          strcpy(mqttHost, mqttH.getValue());
          strcpy(mqttPub, mqttP.getValue());
          strcpy(mqttSub, mqttS.getValue());
          ...
          if(shouldSaveConfig){
            ...
            String(mqttHost), String(mqttPub), String(mqttSub));
            ESP.restart();
          }
          if(!isConnected){
...
void changeCredentials(fs::FS &fs, const char * path, String mailReceiver,
  ...
  File file_ = fs.open(path, "w");
    ...
    config["mqtt"]["host"] = mqttH.c_str();
    config["mqtt"]["root_topic_subscribe"] = mqttS.c_str();
    config["mqtt"]["root_topic_publish"] = mqttP.c_str();
